...
export default function SignupPage() {
    ...
    const handleGoogleSignIn = useGoogleLogin({
        onSuccess: async (tokenResponse) => {
            ...
            try {
                ...
                const errorMsg = error.response?.data?.detail || error.message || "Google sign up failed.";
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const loginRes = await api.post('/login/access-token', loginParams, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                }
            });

            // 3. Update auth context with the new token
    ...
    const onConnectionComplete = useCallback(() => {
        ...
        if (token) {
            toast.success("Welcome to Forsee AI!");
